...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    var res: MutableMap<String, MutableSet<String>>
    val pres = mutableMapOf<String, MutableSet<String>>()
    friends.keys.forEach { it -> pres[it] = friends[it]!!.toMutableSet() }
    do {
        res = pres.toMutableMap()
        res.forEach { (name, friend) ->
            friend.forEach { buddy ->
                if (buddy in res) pres[name] = pres[name]!!.union(res[buddy]!!).toMutableSet()
                else pres[buddy] = mutableSetOf()
            }
        }
    } while (pres != res)
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val char = chars.joinToString(separator = "")
...
fun hasAnagrams(words: List<String>): Boolean {
    val w = mutableMapOf<String, List<Char>>()
    for (word in words) {
        if (w.containsValue(word.toList().sorted())) return true
        else w[word] = word.toList().sorted()
    }
    return false
}
    ...
    if (list.isNotEmpty()) {
        for (i in 0 until list.size) {
            for (a in 1 until list.size) {
                if (list[i] + list[a] == number && i != a) res = i to a
            }
        }
    }
    return res
